...
impl<T: Clone + PartialOrd> LinkedList<T> {
    pub fn merge(mut list_a: LinkedList<T>, mut list_b: LinkedList<T>) -> Self {
        ...
        while a_curr.is_some() || b_curr.is_some() {
            unsafe {
                if let Some(a_ptr) = a_curr {
                    if let Some(b_ptr) = b_curr {
                        // 比较两个链表当前节点的值，并按顺序添加到结果链表中
                        ...
                        // 如果 list_b 已经结束，追加 list_a 的剩余节点
                        result.add((*a_ptr.as_ptr()).val.clone());
                        a_curr = (*a_ptr.as_ptr()).next;
                    }
                } else if let Some(b_ptr) = b_curr {
                    // 如果 list_a 已经结束，追加 list_b 的剩余节点
                    result.add((*b_ptr.as_ptr()).val.clone());
                    b_curr = (*b_ptr.as_ptr()).next;
                }
            }
